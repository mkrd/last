...
${style2.split(";").join(";\n")}}`;
  ...
  function apply_to_element(element) {
    console.log("run apply_to_element", element);
    ...
    for (const mutation of mutation_list) {
      if (mutation.type === "childList") {
        for (const added_node of mutation.addedNodes) {
          if (added_node.nodeType === Node.ELEMENT_NODE && added_node.hasAttribute("ui")) {
            apply_to_element(added_node);
          }
        }
      } else if (mutation.type === "attributes" && (mutation.target.nodeType === Node.ELEMENT_NODE && mutation.target.hasAttribute("ui"))) {
        if (mutation.oldValue === null) {
          continue;
        }
        console.log("mutation", mutation);
        const old_ui_tag = `${mutation.oldValue}`.strip();
        const new_ui_tag = mutation.target.getAttribute("ui").strip();
        console.log(`old: "${mutation.oldValue}"`);
        console.log(`new: "${mutation.target.getAttribute("ui")}"`);
        console.log("eq", mutation.oldValue == mutation.target.getAttribute("ui"));
        if (mutation.oldValue.strip() == mutation.target.getAttribute("ui").strip()) {
          continue;
        }
        apply_to_element(mutation.target);
      }
    }
    time("Mutation observer callback");
  }
