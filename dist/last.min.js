...
${i.split(";").join(`;
`)}}`}).join(`
`),s=document.createElement("style");s.innerHTML=o,document.head.appendChild(s)}};function $(n,t=null){t!==null?console.log("\u{1F4CD} Run apply_to_element from source",t,"for element",n):console.log("\u{1F4CD} Run apply_to_element for element",n);let e=new x([n]);e.make_global_component_style_sheet(),e.apply_component_functions(),lastcss.config.mode==="global"?e.apply_styles_global():lastcss.config.mode==="inline"&&e.apply_styles_inline()}function k(){l("\u{1F7E3}\u{1F3C1} Apply all styles");let n=N(document,"[ui]");n=C(n);for(let t of n)$(t,"apply_all");l("\u{1F7E3}\u{1F3C1} Apply all styles")}function L(n,t){l("Mutation observer callback"),console.log("mutation_list",n);for(let e of n)if(e.type==="childList")for(let o of e.addedNodes)o.nodeType===Node.ELEMENT_NODE&&(!o.hasAttribute("ui")||$(o,"childList mutation"));else if(e.type==="attributes"){if(e.target.nodeType!==Node.ELEMENT_NODE||!e.target.hasAttribute("ui"))continue;let o=String(e.oldValue).trim(),s=e.target.getAttribute("ui").trim();if(o===s){r("\u23ED\uFE0F Skip apply_to_element, ui attribute did not change");continue}$(e.target,"attribute mutation ("+o+" -> "+s+")")}l("Mutation observer callback")}window.lastcss=m;if(!document.body)throw new Error("Unable to initialize Last CSS. Do not use the <script> tag in the header, but rater after the <body> tag");r("\u{1F7E3} Last: start init");l("Last init");y(document,"last:init");k();window.last_css_observer=new MutationObserver(L);window.last_css_observer.observe(document.body,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["ui"],attributeOldValue:!0});y(document,"last:initialized");l("Last init");})();
